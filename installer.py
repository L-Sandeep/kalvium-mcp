...
def chromium_is_installed() -> bool:
    """Return whether Google Chrome is installed for Playwright."""
    chrome_paths = [
    ...
    return any(path.is_file() for path in chrome_paths)


def check_chromium() -> bool:
    """Report whether Google Chrome is installed."""
...
def install_chromium(already_installed: bool) -> None:
    """Verify that Google Chrome is available for Playwright."""
    if already_installed:
        success("Skipping Chromium installation; Google Chrome is available")
        return

    raise SetupError(
        "Google Chrome is not installed. Install Google Chrome, then run "
        "the installer again."
    )
